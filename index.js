...
console.log(MenuElemaniOlustur("Hamburger", 150, "Burgerler"));
console.log(MenuElemaniOlustur("Pizza", 120, "Pizzalar"));
console.log(MenuElemaniOlustur("Kola", 30, "Soğuk İçecekler"));
...
const burger = {
  ...
  indirim: function (indirimOrani) {
    if (indirimOrani === "öğretmen") {
      return this.fiyat * 0.75;
    } else if (indirimOrani === "öğrenci") {
      return this.fiyat * 0.75;
...
const ogrenciIndirimliFiyat = burger.indirim("öğrenci");
console.log("Öğrenci indirimli fiyat:", ogrenciIndirimliFiyat);
...
*/
const hedefIsim = "Ahmet";
let ahmetinGeriBildirimi = null;
for (let i = 0; i < degerlendirmeler.length; i++) {
  if (degerlendirmeler[i].isim === hedefIsim) {
    ahmetinGeriBildirimi = degerlendirmeler[i].geribildirim;
    break;
  }
}
if (ahmetinGeriBildirimi !== null) {
...
const yeniDegerlendirme = DegerlendirmeEkle(
  degerlendirmeler,
  "Mert",
  5,
  "Buranın her şeyine bayıldım. Tüm yemekler çok lezzetliydi"
);
...
function SonDegerlendirmeyiAl(degerlendirmeler) {
  const sonDegerlendirme = degerlendirmeler[degerlendirmeler.lenght - 1];
  if (sonDegerlendirme) {
